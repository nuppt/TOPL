...
(* as1: assert the state of X is 3. *)
Definition as1 : Assertion := fun st => st X = 3.
(* as2: assert the state of X is equal or less than Y. *)
Definition as2 : Assertion := fun st => st X <= st Y.
(* as3: assert the state of X is 3 or the state of X is equal or less than Y *)
Definition as3 : Assertion :=
  fun st => st X = 3 \/ st X <= st Y.
(* Just simplify (st X) to be X, similarily (st Z) to be Z.
   as4: assert Z * Z <= X and (Z + 1) * (Z + 1) > X， so Z is equal sqrt(X). *)
Definition as4 : Assertion :=
  fun st => st Z * st Z <= st X /\
            ~ (((S (st Z)) * (S (st Z))) <= st X).
(* as5: assert true forever. *)
Definition as5 : Assertion := fun st => True.
(* as6: assert false forever. *)
...
(** This way of writing assertions can be a little bit heavy,
    for two reasons: 
(1) every single assertion that we ever write is
    going to begin with [fun st => ]; 
(2) this state [st] is the
    only one that we ever use to look up variables (we will never need
    to talk about two different memory states at the same time). 

For discussing examples informally, we'll adopt some simplifying
...
(** Paraphrase the following Hoare triples in English.
   1) {{True}} c {{X = 5}}
No matter what the state is before executing c, the assertion "X = 5" will be hold after executing c.
   2) {{X = m}} c {{X = m + 5)}}
Before executing c, X = m is hold. After executing c, X = m + 5 is hold.
   3) {{X <= Y}} c {{Y <= X}}
Before executing c, X <= Y is hold. After executing c, Y <= X is hold.
   4) {{True}} c {{False}}
No matter what the state is before executing c, there is no assertion hold about state after executing c. In another word, There is no st st',
c / st || st'.
   5) {{X = m}} 
      c
      {{Y = real_fact m}}.
Before executing c, X = m is hold. After executing c, Y = real_fact m will be hold.
   6) {{True}} 
      c 
      {{(Z * Z) <= m /\ ~ (((S Z) * (S Z)) <= m)}}
No matter what the state is before executing c, after executing c, the assertion "Z * Z <= m" and "(Z + 1) * (Z + 1) <= m" will both be hold.
 *)
...
(** Which of the following Hoare triples are _valid_ -- i.e., the
   ...
   7) {{True}} WHILE True DO SKIP END {{False}}  invalid.

   8) {{X = 0}}
      WHILE X == 0 DO X ::= X + 1 END  valid.
      {{X = 1}}
...
(* Post condition is true forever means no matter what the state is after executing c.
   In another word, no matter what the state is after executing c, it is always been accepted.
*)
Theorem hoare_post_true : forall (P Q : Assertion) c,
  (forall st, Q st) ->
  {{P}} c {{Q}}.
Proof.
  intros P Q c H. unfold hoare_triple. (* Note {{P}} c {{Q}} is a symbol of hoare_triple *)
  intros st st' Heval HP.
  apply H.  Qed.

(* With the corresponding of hoare_post_true, there is a situation that pre condition is false forever.
   In another word, no matter what the state is after executing c, I don't case, the hoare triple is just valid. Because I know there is no assertion of any property about st in P to be hold, remember "False is nothing".
*)
...
(* 针对assignment语句的代换变换:
   P 是对X的状态的assertion， 这里通过update语句，把X的状态改成了a的状态，就得到了一个关于a的状态的assertion。
 *)
...
Theorem hoare_asgn : forall Q X a,
  {{Q [X |-> a]}} (X ::= a) {{Q}}.
Proof.
  unfold hoare_triple. (* 解开定义 *)
  intros Q X a st st' HE HQ.
  (* 对HE做inversion，HE若成立，则根据ceval中的定义，把st'按规则展开。 *)
  inversion HE. subst.
...
(* counterexample:
   {{ True }}
   X ::= X + 1     (* a can be any arithmetic expression. *)
   {{ X = X + 1 }}
*)
...
Proof.
  ...
  exists (st X).
  apply conj.
  assert(H_update_shadow: update (update st X (aeval st a)) X (st X) = st).
  - apply functional_extensionality. intros.
    rewrite update_shadow.
    rewrite update_same.
    reflexivity.
    reflexivity.
  - rewrite H_update_shadow.
    apply H0.
  - (* 为啥还需要assert H_update_shadow，前面的H_update_shadow哪去了？ *)
    assert(H_update_shadow: update (update st X (aeval st a)) X (st X) = st).
    apply functional_extensionality. intros.
    rewrite update_shadow.
    rewrite update_same.
...
Proof.
  (* 利用hoare_consequence_pre来证明。其中：
     P : fun st => True
     为了得到{{ True }} ->> {{ 1 = 1 }} ...
     的形式，我们需要把{{ 1 = 1 }}形式化地描述出来。
     P' := (fun st => st X = 1) [X |-> ANum 1]).
   *)
...
Proof.
  ...
  Case "left part of seq".
    ...
    intros st H. subst.
    (* unfold assn_sub. unfold update. simpl. Qed. *)
    reflexivity. Qed.
...
Proof.
  ...
  eapply hoare_consequence_pre.
  apply hoare_asgn.
  intros st H. (* just "intros." would do nothing. *)
  unfold assn_sub; simpl. split; reflexivity.
Qed.
(** [] *)
...
(* 反例： a = AId X， 且 n <> 3.
   假设反例的Prop可以被证明。
   由hoare_seq规则展开。
   由hoare_assn规则，(Y ::= a) {{fun st => st Y = n}}的前条件应该是
   {{fun st => aeval st a = n}}，记为Q.
   由hoare_assn规则，(X ::= (ANum 3)) {{fun st => aeval st a = n}}的前条件应该是
   {{fun st => aeval st a = n /\ st X = 3}}。
   所以如果上面的hoare_triple是valid，由hoare_consequence_pre古泽，必须有下面的霍尔蕴含式成立：
   {{a = AId X /\ n <> 3}} ->> {{fun st => aeval st a = n /\ st X = 3}}，化简一下就是：
   {{ n <> 3 }} ->> {{ n = 3 }}，即
   forall st, n <> 3 -> n = 3.
   显然上面的Prop不成立，所以反例的Prop不成立，所以原来的Prop有反例，不可以被证明。
 *)
...
Theorem hoare_if1 : forall P Q b c,
  ...
  {{P}} (IF1 b THEN c FI) {{Q}}.
Proof.
  intros P Q b c H1 H2 st st' HE HQ.
  inversion HE; subst.
  apply (H1 st st').
  apply H6.
  apply conj.
      apply HQ.
      unfold bassn. apply H3.
  apply (H2 st').
  apply E_Skip.
  apply conj.
      apply HQ.
...
Proof. (* FILL IN HERE *)
  ...
  intros.
  inversion H; subst.
  unfold not, bassn in H0.
  simpl in H0.
  destruct H0.
  SearchAbout beq_nat.
  remember (beq_nat (st' Y) 0).
  destruct b.
  symmetry in Heqb.
...
Proof.
  ...
  eapply E_Repeat.
  eapply E_Seq.
      apply E_Ass. reflexivity. (* 尝试接受存在量词： 1 = ?2434，并带入下面的subgoal *)
 ...
 *)
(* 我们一步一步推导：
如果上面的Hoare_triple是valid，那么
对于任意 Q : Assertion,有下面两个Hoare_triple是valid的：
    1. {{ X > 0 }} (Y ::= X;; X ::= X - 1) {{Q}}
    2. {{Q}}
       WHILE (X <> 0) DO
           Y ::= X;; X ::= X - 1
...
我们得到
    ...
    END
    {{ X = 0 /\ Y > 0 }}
由Hoare_while，我们能够根据该while loop的前条件推出后条件：
    P /\ ~ (X <> 0) <=> X >= 0 /\ Y > 0 /\ X = 0 <=> X = 0 /\ Y > 0
所以原来的hoare triple是valid的。
 *)
