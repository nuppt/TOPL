...
(* id是identifer，id类型定义如下，简单的说：
   1. 一个id类型的对象它由构造子Id来定义，给Id函数一个nat，它就可以返回给我们一个id类型的对象，比如下面例子都是id类型的程序对象：
     Id 0
     Id 1
     Id 2
     Id 100
     Id x
   2. 显然，我们由函数的单射性可以知道，给Id函数不同的n，返回的id类型对象Id n是不同的。
   3. 后面，我们将要用id类型的对象作为我们设计的PL中的程序变量。
 *)
Inductive id : Type :=
  Id : nat -> id.

(* 定理意义： 对于任意两个id类型的对象，它们要么属于同一个id类型对象pair的集合，要么属于不同id类型对象pair的集合。它们（这两种情况）分别被定义为true和false。（什么叫被定义为true和false？ 看sumbool部分。）
   简单的说，就是两个变量，要么指向同一个内存单元（抽象的），要么指向不同内存单元。
 *)
...
(* 定理意义： x和x显然是同一个id类型对象，它们属于同一个id类型对象pair的集合，故应该返回p。
 *)
...
(* 定理意义： x和y是不同的id类型对象，即x<>y，故(eq_id_dec x y)返回false，所以返回else分支，得到q。 *)
...
(* 为id类型的对象，定义属性state：
   给state一个id对象，就返回一个nat（该id对象的value）。
 *)
Definition state := id -> nat.

(* 定义一个特殊状态： empty_state —— 不管给什么id类型对象，只返回状态 0。 *)
Definition empty_state : state :=
  fun _ => 0.

(* 下面我们把id类型对象全部叫做内存变量。。。
   定义内存变量的写操作 update： 把内存变量x的状态(st x)更新为n。
   具体定义如下：
      对于整个内存空间（Id 0, Id 1, Id 2, ..., Id MAX）上的任一变量x'，
      1. 如果 x' = x，返回n;
      2. 如果 x' <> x，返回(st x).
   这样我们就完成了对内存变量x的值的改写。
 *)
...
(* 定理意义： 更新内存变量x的状态(st x)为n，更新后查看x的状态(值)必然是n。 *)
...
(* 定理意义： 更新内存变量x2的状态(st x2)为n，更新后查看另一个内存变量x1的状态(st x1)，显然就是(st x1)本身。 *)
...
(* 定理意义： 把(Id 2)的状态更新为n，而其他所有的内存空间中的变量全部更新为0。
            之后，查询内存变量(Id 3)的状态，必然等于0。
 *)
...
(* 定理意义： 对同一个内存变量，它当前的状态由它最后一次的状态更新决定。
      等式左边，先把内存变量x2的状态更新为n1，再把x2的状态更新为n2，然后查看内存变量x1的状态。
      等式右边，直接把x2的状态更新为n2，然后查看内存变量x1的状态。
   Note： 两者在查看x1状态的时候，整个内存空间的状态相同。
 *)
...
(* 定理意义： 某一个内存变量，修改前后的状态相同，那么整个内存空间的状态集合相同。
   为什么说整个内存空间地状态集合相同呢，因为x2是任意的，故(st x2)是整个内存空间上每一个内存变量的状态。
 *)
...
(* 定理意义： 对两个不同的内存变量的状态做修改，更新后的全局状态（整个内存空间）不依赖于状态更新的先后顺序。
  即对于两个不同的内存变量x1和x2 (x1 <> x2)，
     1. 先更新x1为n2，后更新x2为n1;
     2. 先更新x2为n1，后更新x1为n2.
  得到的全局状态(forall x3, st x3)是相同的。
 *)
...
Proof.
  ...
  Case "x1 = x3".
    ...
    SCase "x2 = x3".
      apply ex_falso_quodlibet; apply H; rewrite e; rewrite e0; reflexivity.
    SCase "x2 <> x3".
      reflexivity.
  Case "x1 <> x3".
    destruct (eq_id_dec x2 x3).
...
(* 定义算术表达式类型 —— 即“什么是一个算术表达式”？ 
      一个算术表达式（程序）对象可以由下面几种构造子定义，它们符合BNF范式，且每一个constructor的返回值（最后一个类型参数）必然是aexp类型。
   对aexp说明如下：
   一个算术表达式，它
      1. 要么是(n:nat), (ANum n);
      2. 要么是(X:id), (AId X);
      3. 要么是(x,y:aexp), (APlus x y);
      4. 要么是(x,y:aexp), (AMinus x y);
      5. 要么是(x,y:aexp), (AMult x y);
      6. 除上面之外，没有其他情况。
*)
Inductive aexp : Type :=
  | ANum : nat -> aexp
...
(* 虽然我们上面都把内存变量的状态和变量的值看作是一个概念，但本质上两者还是由区别的：
      1. 内存变量的状态是计算机世界的概念，我们可以把任意一个计算机世界中的对象的属性纳入状态的范畴，这个属性有它自有的类型和相应的值 —— 这里我们把内存变量的存储属性值作为内存状态；
      2. 变量的值是数学世界的概念，它本身不依赖于存储方式，是作为一个抽象概念存在。
   这里我们已经知道，内存变量的存储属性值就作为内存状态，那么我们构造算术运算结构的话，就不必要用内存变量状态的概念，而是直接用算术值的概念。
   下面就是内存变量状态 -> 变量值的映射函数aeval。
 *)
...
(* 定义过程式PL中的程序命令，其中有控制流命令，也有赋值命令:
   一个com类型的对象，它
      1. 要么是CSkip;
      2. 要么是(X:id, ae:aexp), (CAss X ae);
      3. 要么是(c1,c2:com), (CSeq c1 c2);
      4. 要么是(be:bexp, c1,c2:com), (CIf be c1 c2);
      5. 要么是(be:bexp, c:com), (CWhile be c);
      6. 除上面case之外，没有其他case。
 *)
Inductive com : Type :=
  | CSkip : com
  | CAss : id -> aexp -> com
...
(* 先对X和Z赋初值，然后执行函数体subtract_slowly。
   可以看到，通过不断地Definition代码块，我们可以构造出越来越大，功能也越来越具体，强大的代码，这点和其他PL中的做法一样，组合各种模块，完成我们的计算任务。
 *)
...
(** *** An infinite loop: *)
(* 定义一个dead loop： 它啥都不做，就是一直不断地循环下去。 *)
Definition loop : com :=
  WHILE BTrue DO
    SKIP
  END.
...
(* st是执行c之前的程序状态，ceval_fun_no_while返回执行c之后的状态。 *)
Fixpoint ceval_fun_no_while (st : state) (c : com) : state :=
  match c with
      (* 如果c是SKIP，即程序什么都没做，原全局状态st不变，直接返回。 *)
    | SKIP =>
        st
      (* 如果c是x::=a1，即把a1的值赋值给变量x(注意：a1可能是变量，也可能只是数值，统一为aexp类型)，更新x的状态，其余内存状态不变。 *)
    | x ::= a1 =>
        update st x (aeval st a1)
      (* 如果c是c1;;c2，即先执行c1命令，再执行c2命令，引入中间状态st'，这样我们就可以按序刻画状态的变化。 *)
    | c1 ;; c2 =>
        let st' := ceval_fun_no_while st c1 in
        ceval_fun_no_while st' c2
      (* 如果c是IFB控制流命令，根据对b状态的判断，来选择对哪一条命令的执行。 *)
...
Inductive no_whilesR: com -> Prop :=
  ...
  | NW_Ass : forall i ae, no_whilesR (CAss i ae)
  | NW_Seq : forall c1 c2, (no_whilesR c1) -> (no_whilesR c2) -> (no_whilesR (CSeq c1 c2))
  | NW_If : forall be c1 c2, (no_whilesR c1) -> (no_whilesR c2) -> (no_whilesR (CIf be c1 c2)) .
...
(* FILL IN HERE *)
(** [] *)
(* 初始状态为st，对于任意一个不含while loop的command，我们都可以找到一个终止状态st'。 *)
...
Proof.
  ...
  Case "c: Ass".
    intros.
    exists (update st i (aeval st ae)).
    apply E_Ass.
    reflexivity.
  Case "c: Seq".
    intros.
  ...
  Case "c: IF".
    intros.
    remember (beval st be) as cond. (* 不remember的话，destruct会直接丢弃该cond *)
    destruct cond.
    SCase "st be = true".
      destruct (IHno_whilesR1 st).
      exists x.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  match prog with
    ...
    | s :: prog' => match s with
                      ...
                      | SLoad n => s_execute st ((st n) :: stack) prog'
                      | SPlus => match stack with
                                   | x :: y :: stack' => s_execute st ((plus y x) :: stack') prog'
                                   | _ => s_execute st stack prog'
                                 end
                      | SMinus => match stack with
                                    | x :: y :: stack' => s_execute st ((minus y x) :: stack') prog'
                                    | _ => s_execute st stack prog'
                                  end
                      | SMult => match stack with
                                   | x :: y :: stack' => s_execute st ((mult y x) :: stack') prog'
                                   | _ => s_execute st stack prog'
                                 end
                    end
  end.
...
Lemma s_execute_cons:
  forall (st : state) (stack : list nat) (ins : sinstr) (l : list sinstr),
    s_execute st stack (ins :: l) = s_execute st (s_execute st stack [ins]) l.
Proof.
  induction stack.
  - intros.
    destruct ins; auto.
  - intros.
    destruct ins; destruct stack; auto.
Qed.

Lemma s_execute_compose:
  forall (st : state) (stack : list nat) (l1 l2 : list sinstr),
...
Proof.
  ...
  - rewrite <- app_comm_cons.
    intros.
    rewrite s_execute_cons.
    rewrite IHl1.
    rewrite <- s_execute_cons.
    reflexivity.
Qed.
Theorem s_compile_cor : forall (st : state) (e : aexp) stk,
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  (* FILL IN HERE *)
  | E_Break : forall st, (* 执行break，st保持不变，产生SBreak信号 *)
      CBreak / st || SBreak / st
  | E_Ass : forall st a1 n x,
      aeval st a1 = n ->
      (x ::= a1) / st || SContinue / (update st x n)
  | E_IfTrue : forall st st' b c1 c2 stu,
      beval st b = true ->       
      c1 / st || stu / st' ->
      (IFB b THEN c1 ELSE c2 FI) / st || stu / st'
  | E_IfFalse : forall st st' b c1 c2 stu,
      beval st b = false ->
      c2 / st || stu / st' ->
      (IFB b THEN c1 ELSE c2 FI) / st || stu / st'
  | E_Seq_c1Break : forall c1 c2 st st',
      c1 / st || SBreak / st' ->
      (c1 ;; c2) / st || SBreak / st'
  | E_Seq_c1Continue : forall c1 c2 st st' st'' stu,
      c1 / st || SContinue / st' ->
      c2 / st' || stu / st'' ->
      (c1 ;; c2) / st || stu / st''
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "E_IfTrue" | Case_aux c "E_IfFalse"
  | Case_aux c "E_Seq_c1Break" | Case_aux c "E_Seq_c1Continue"
  | Case_aux c "E_WhileEnd" | Case_aux c "E_WhileLoop"
