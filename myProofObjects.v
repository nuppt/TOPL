...
(* 函数 in Computational universe 的定义语法:
       Definition fun_name (list of parameters) : (return_value) :=
         function body.

   函数 in Logic universe 的定义语法：
       Definition log_name : forall parameters, Prop1 -> ... -> Propn :=
         fun para1 => fun para2 => ... => fun Prop1 => ... => fun Propn => evidences.

   上面的两个定义中，有几点说明，来表明它们的一致性：
       1. Computational universe中的list of parameters，内含了全称量词，并且parameters不仅可以是data，还可以是函数 —— 例子b_plus3''中，我们就把函数beautiful n 作为一个parameter。对于函数，我们还可以从另一个角度去看（以例子来讲解）：beautiful n 除了可以看作带有一个参数n的函数beautiful，还可以看作一个Prop —— n具有beautiful性质，或者说n是具有beautiful性质的数。
       2. 在Computational unverse中，function body 就是Logic universe中的 evidences。
       3. 在Logic universe中，匿名函数fun的映射关系（运算）是右结合的，那么对于list of parameters，和在Computational universe中的映射层次是一致的。怎么理解呢：
       fun_name para1 == log_name para1.
       fun_name para1 para2 == log_name para1 para2.
       ...
 *)
...
Definition and_commut_fact: forall P Q, P /\ Q -> Q /\ P :=
  fun (P Q : Prop) (H : P /\ Q) =>
     match H with
     | conj HP HQ => conj Q P HQ HP
     end.
...
(* conj P Q proofs_of_P proofs_of_Q*)
(** [] *)
