...
(** In Coq, propositions have the same status as other types, such as
    [nat].  Just as the natural numbers [0], [1], [2], etc. inhabit(翻译为：归于,就像一个分类树一样，nat被分为了[0],[1],...)
...
Proof.
  (* WORKED IN CLASS *)
  intros P Q H.
  destruct H as [HP HQ]. (* 对条件Prop进行拆分 *) 
  split.                 (* 对目标Prop进行拆分 *)
    Case "left". apply HQ. 
...
Proof.
  intros P Q R H.
  destruct H as [HP [HQ HR]]. (* 解构嵌套结构 *)
  (* FILL IN HERE *)
  split.
  Case "left P /\ Q".
    split.
    SCase "left P". apply HP.
    SCase "right Q". apply HQ.
  Case "right R".
    apply HR.
  (** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros P Q R H1 H2.
  destruct H1 as [HPQ HQP].
  destruct H2 as [HQR HRQ].
  split.
  Case "->".
    intros HP.
    apply HPQ in HP.
